...
ITER_SIZE = 1
NUM_WORKERS = 4
DATA_DIRECTORY = './dataset/gta5_list'
DATA_LIST_PATH = './dataset/gta5_list/train.txt'
IGNORE_LABEL = 255
INPUT_SIZE = '1280,720'
DATA_DIRECTORY_TARGET = './dataset/cityscapes_list'
...
POWER = 0.9
RANDOM_SEED = 1234
RESTORE_FROM = './model/GTA2Cityscapes_vgg-ac4ac9f6.pth'
SAVE_NUM_IMAGES = 2
SAVE_PRED_EVERY = 5000
SNAPSHOT_DIR = './snapshots/source_only/'
WEIGHT_DECAY = 0.0005
...
def main():
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            ############################################################
            #  1. Please write your code from here:
            # train with source
            ############################################################
            ...
            pred1 = model(images)
            pred1 = interp(pred1)
            loss_pred1 = loss_calc(pred1, labels, args.gpu)
            loss = loss_pred1
            loss.backward()
            #loss_seg_value1 += list(loss.data.cpu().numpy())[0]


            # ############################################################
            # #  2. Please write your code from here:
            # # train with target
            # ############################################################
            _, batch = targetloader_iter.__next__()
            target_images, _, _ = batch
            target_images = Variable(target_images).cuda(args.gpu)
            pred_target1 = model(target_images)
            ...
            loss.backward()
            #loss_adv_target_value1 += loss_adv_target1.data.cpu().numpy()[0]
            ...
            # ############################################################
            # #  3. Please write your code from here:
            # # train with source
            # ############################################################
            pred1 = pred1.detach()
        ...
        print('iter = {0:8d}/{1:8d}, loss_seg = {2:.3f} loss_adv = {4:.3f}, loss_D = {6:.3f}'.format(
